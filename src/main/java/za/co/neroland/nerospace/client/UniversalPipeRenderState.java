...
/**
 * Render state for the Universal Pipe: which arms are connected, which resource layers currently
 * hold content (drives the coloured streams), and the items physically travelling through the
 * segment (position pre-computed during extraction). {@code itemStates} entries are pooled and
...
public class UniversalPipeRenderState extends BlockEntityRenderState {
    ...
    /** ARGB colour per stream layer (fluid/gas tint to their contents). */
