 ...
 *
 * Destination buttons are built for the full destination order and enabled per the live synced mask.</p>
 */
public class RocketScreen extends TexturedContainerScreen<RocketMenu> {
    ...
    private static final List<ResourceKey<Level>> ALL_DESTINATIONS = za.co.neroland.nerospace.rocket.Destinations.all();
    ...
    protected void init() {
        ...
        for (int i = 0; i < ALL_DESTINATIONS.size(); i++) {
            final int index = i;
            SpaceButton node = new SpaceButton(x, this.topPos + 36, 28, 14,
