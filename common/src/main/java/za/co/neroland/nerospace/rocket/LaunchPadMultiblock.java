 ...
 * forms the pad footprint a rocket is fuelled on. A complete {@code 3x3} square is the canonical pad,
 * recognised so adjacent machinery can reward it; a {@code 5x5} with a Launch Gantry on its ring is
 * the Heavy Launch Complex.
 *
 * <p>This class is pure geometry: it never mutates the world, so it is safe to call from a tick.</p>
 */
public final class LaunchPadMultiblock {
    ...
    /**
     * Like {@link #fullSquareCorner(Set, int)} but the square must CONTAIN {@code pos} (XZ) — used by
     * launch gating so a rocket is grounded when the square it actually stands on degrades.
     */
    @Nullable
    ...
    /**
     * The Heavy Launch Complex: a complete aligned 5x5 pad with at least one Launch Gantry module on
     * its border ring at pad level. A Heavy complex deploys/launches Tier 3 without the Station-Wall ring.
     */
    ...
    /** Tier 3 gating: whether some complete 3x3 in {@code pads} is ringed with Station Wall. */
