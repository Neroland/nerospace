...
/**
 * Menu for the quarry controller. Combined machine inventory layout: slot 0 = frame casing,
 * 1..M = module cards, then the output buffer; followed by the player inventory. Status (energy,
 * state, fluid, depth) is synced through {@link ContainerData}.
 *
 * <p>The client constructor assumes a Tier-1 layout (1 module slot). Higher tiers will pass the slot
 * count through the open buffer when they ship — for now only Tier 1 is registered.</p>
 */
public class QuarryMenu extends AbstractContainerMenu {
    ...
    public QuarryMenu(int containerId, Inventory playerInventory, Container container, ContainerData data, int moduleSlots) {
        ...
        // Frame casing slot, then module slots along the top row.
    ...
    public ItemStack quickMoveStack(Player player, int index) {
        ...
        if (index < this.machineSlots) {
            ...
            // Player inventory -> frame slot or module slots (output rejects placement).
