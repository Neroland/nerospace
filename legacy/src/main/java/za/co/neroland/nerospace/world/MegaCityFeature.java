...
/**
 * Living Mega-City (ALIEN_VILLAGERS_DESIGN.md §5.2 / §7 — redesigned) — the massive end-state alien
 * settlement: a pillared, lit, crenellated curtain wall with four gates around a glowing tile plaza of
 * futuristic towers, and a central keep guarded by the {@link za.co.neroland.nerospace.entity.RuinWarden}
 * boss over a grand vault. Very rare and spaced via {@link StructureSpacing}.
 */
public class MegaCityFeature extends Feature<NoneFeatureConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> ctx) {
        ...
        // Plaza floor + curtain wall (pillars every 5, crenellations, lamps, gated mid-sides).
        for (int dx = -WALL_R; dx <= WALL_R; dx++) {
            for (int dz = -WALL_R; dz <= WALL_R; dz++) {
                ...
                // Crenellations + pillar-cap lamps.
                if (pillar) {
